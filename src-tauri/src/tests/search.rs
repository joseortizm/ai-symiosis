...
#[test]
fn test_search_performance_consistency() {
    // Test that repeated searches have consistent performance
    let query = "test";
    let iterations = 5;
    ...
    for _ in 0..iterations {
        let start = Instant::now();
        let _result = search_notes_hybrid(query, 100);
        let duration = start.elapsed();
    ...
    // Maximum should not be more than 3x the minimum (allowing for system variance)
    assert!(
        *max_duration <= *min_duration * 3,
        "Search performance is inconsistent: min={}ms, max={}ms, avg={}ms",
        min_duration,
        max_duration,
        avg_duration
    );
    ...
    assert!(
        avg_duration < 500,
        "Average search performance is too slow: {}ms",
        avg_duration
    );
}
