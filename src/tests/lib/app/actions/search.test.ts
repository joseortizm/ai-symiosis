...
describe('search actions', () => {
  ...
  describe('updateFilteredNotes', () => {
    it('should update filtered notes in search manager', () => {
      ...
      expect(mockDeps.searchManager.setFilteredNotes).toHaveBeenCalledWith(notes);
    });
  ...
  describe('edge cases', () => {
    ...
    it('should handle very large note arrays', () => {
      const largeNoteArray = Array.from({ length: 10000 }, (_, i) => `note${i}.md`);
      
      expect(() => {
        searchActions.updateFilteredNotes(largeNoteArray);
      }).not.toThrow();
      
      expect(mockDeps.searchManager.setFilteredNotes).toHaveBeenCalledWith(largeNoteArray);
    });
  });
});