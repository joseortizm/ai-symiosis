...
import { getHighlightedContent, clearHighlightCache } from '$lib/core/contentHighlighting.svelte';
...
describe('contentHighlighting', () => {
  ...
  describe('getHighlightedContent', () => {
    ...
    it('should highlight multiple occurrences case-insensitively', () => {
      ...
      const result = getHighlightedContent(content, 'world', false);
      expect(result).toBe('Hello <mark class="highlight">WORLD</mark> and <mark class="highlight">world</mark> again');
    });
  ...
  describe('caching behavior', () => {
    ...
    it('should use cache key based on content prefix and full query', () => {
      ...
      const longContent = 'Hello world with much more content that extends beyond the cache key prefix length of 100 characters to test that different content with same prefix uses different cache entries';
      const query = 'world';
  ...
  describe('edge cases', () => {
    ...
    it('should handle HTML-like content safely', () => {
      ...
      const result = getHighlightedContent(content, 'world', false);
      expect(result).toBe('<div>Hello <span><mark class="highlight">world</mark></span></div>');
      ...
      const maliciousResult = getHighlightedContent(maliciousContent, 'world', false);
      expect(maliciousResult).toBe('<script>alert("xss")</script><mark class="highlight">world</mark>');
